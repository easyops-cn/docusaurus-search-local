...
| `searchIndexBaseUrl`              | string                                                                      |           | Base URL from which search index files are served. Useful when your site assets are hosted on a separate CDN domain (different origin). The CDN must respond with an appropriate `Access-Control-Allow-Origin` header. See [CDN / different-origin assets](#cdn--different-origin-assets) for details.                                                       |
